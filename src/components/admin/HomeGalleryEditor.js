...
export default function HomeGalleryEditor() {
  ...
  const currentSig = useMemo(() => galleryHomeContentSignature({ version: savedVersion, images }), [savedVersion, images]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const data = await loadGalleryHomeContent();
        if (cancelled) return;
        setSavedVersion(data.version);
        setImages(data.images);
        setBaselineSig(galleryHomeContentSignature(data));
        setStatus('ready');
      } catch (e) {
        if (cancelled) return;
        ...
        setBaselineSig(galleryHomeContentSignature(def));
        setLoadError(
          e instanceof Error
  ...
  async function persistManifest(nextImages, nextVersion) {
    ...
    const doc = sanitizeGalleryHomeContentForSave({ version: nextVersion, images: nextImages });
    setSaving(true);
    setSaveError(null);
    try {
      const result = await saveGalleryHomeManifest(token, doc);
      if (result.ok) {
        setSavedVersion(doc.version);
        setImages(doc.images);
        setBaselineSig(galleryHomeContentSignature(doc));
  ...
  async function onUpload(file) {
    ...
    setUploading(true);
    try {
      const req = await requestGalleryHomeUpload(token, {
        filename: file.name,
  ...
  async function onDelete(filename) {
    ...
    const ok = await persistManifest(nextImages, savedVersion + 1);
    if (!ok) return;
    const del = await deleteGalleryHomeImage(token, filename);
    if (!del.ok) {
  ...
  return (
    <div className="admin-bridal-form">
      <h3 className="admin-panel-title">Home gallery</h3>
      <p className="admin-muted">
        Images for the home page grid. Stored in S3 under <code>gallery-home/</code>. Reorder with the arrows, then
        save. Uploads apply immediately.
      </p>
      {loadError ? (
      ...
      <div className="admin-gallery-upload">
        <label className="admin-label" htmlFor="home-gallery-upload">
          Add image
        </label>
        <input
          ref={fileInputRef}
        ...
        />
        <p className="admin-hint admin-muted">JPEG, PNG, or WebP. Up to {MAX_IMAGES} images.</p>
        ...
        <ul className="admin-gallery-list">
          {images.map((filename, index) => (
            <li key={filename} className="admin-gallery-item">
              <img
                src={galleryHomeImageUrl(filename)}
                alt=""
